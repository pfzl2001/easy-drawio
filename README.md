...
由于本扩展是本地开发版本，您需要通过浏览器的“开发者模式”来加载：
...
5. 在弹出的本地文件选择器中，选中本项目的 `extension` 文件夹。
6. 加载成功后，强烈建议您点击浏览器工具栏的“拼图”图标，将 **Easy Draw.io** 固定 (Pin) 在地址栏旁边。

## 💡 使用指南

- **打开画板**：在任何正在浏览的网页上，点击地址栏旁边的本插件图标即可召唤右侧边栏。 
- **手动保存**：虽然插件会自动保存，但您也可以随时使用快捷键 `Ctrl + S` （或 `Cmd + S`）手动保存，或者点击顶部工具栏的保存按钮。
- **重新开始**：点击顶部工具栏的第一个按钮 (New Diagram) 可以清空当前标签页隔离环境下的草稿，重新创建一张空图。
- **导入代码**：点击顶部工具栏最右侧的 `</>` 按钮打开导入面板，在下拉菜单选择您的代码类型 (`Draw.io XML`, `Mermaid`, `PlantUML`)，粘贴代码并点击 Insert 即可渲染。

## 🛠️ 技术栈说明

- **Manifest V3**
- Vanilla JavaScript / CSS / HTML 组合构建
- `window.postMessage` 官方 Embed Protocol 通信协议
- 极速启动：Iframe 调用了官方的嵌入式前端（包含极简 UI 与 JSON 协议）
